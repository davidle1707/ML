...
namespace SharedCache.WinServiceCommon.Sockets
{
	...
	public class SharedCacheTcpClient : IDisposable
	{
		...
		public SharedCacheTcpClient(string host, int port)
		{
			...
			IPEndPoint endpoint = new IPEndPoint(ParseIpAddress(this.host), this.port);
		...
		private IPAddress ParseIpAddress(string hostNameOrAddress)
		{
			...
			try
			{
				return Dns.GetHostEntry(hostNameOrAddress).AddressList[0];
			}
			catch
			{
				throw;
