...
using Entity = WorldofWatches.DataContext;
using EntityInc = WorldofWatches.DataContext.Inc;
...
namespace WorldofWatches.Console
{
    internal class ProductManager
    {
        ...
        private readonly Entity.WOWDataContext _db;
        ...
        public async Task<ObjectId?> SaveAsync(Product product)
        {
            ...
            var existEntity = await _db.Products.Find(p => p.Key == entity.Key).Project(p => new { p.Id, p.Type }).FirstOrDefaultAsync();
            entity.Id = existEntity?.Id ?? entity.Id;

            if (existEntity != null && !existEntity.Type.StartsWith("Unknow"))
            {
                entity.Type = existEntity.Type;
            }
        ...
        public async Task<ObjectId?> UpdatePriceAsync(Product product)
        {
            ...
            if (!entity.Price.Amount.Equals(product.Price.Amount) || entity.Price.ListAmount != product.Price.ListAmount || entity.Price.RegAmount != product.Price.RegAmount)
            {
        ...
        private void ConfigAutoMapper()
        {
            ...
            //Mapper.CreateMap<ProductSpecification, EntityInc.ProductSpecification>();
