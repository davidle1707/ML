...
namespace ML.Common
{
	// refer: https://gist.github.com/sprycoder/a14cce9979ceef61cc70523ed00a0b55
	public static class CurrencyToWord
    {
		...
		/// <returns></returns>
		public static string ToWords(this decimal value, string firstPrefix = "Dollars", string secondPrefix = "Cents")
		{
			string decimals = "";
			...
			string strWords = GetWords(input) + (string.IsNullOrWhiteSpace(firstPrefix) ? firstPrefix : $" {firstPrefix}");

			if (decimals.Length > 0)
			{
				// if there is any decimal part convert it to words and add it to strWords.
				var secondWords = GetWords(decimals);
				strWords += " and " + (string.IsNullOrWhiteSpace(secondWords) ? "00" : secondWords) + (string.IsNullOrWhiteSpace(secondPrefix) ? secondPrefix : $" {secondPrefix}");
			}
