...
using ML.Utils.OnlineFantasyData.Providers.NumberFire.NFL;
...
{
	internal sealed class NflHelper : __BaseHelperByHtml
	{
		public NflHelper(NumberFireProvider provider) : base(provider)
		{
		}
		
		#region Daily Lineups: https://www.numberfire.com/nfl/daily-fantasy
			...
			try
			{
				...
				using (var client = new WebClient())
				{
					...
					foreach (var site in referSites)
					{
						...
						var content = client.UploadString(new Uri("https://www.numberfire.com/nfl/daily-fantasy/get-dashboard"), "s=" + site);
		...
		{
			return string.Format("NFL_DailyLineups_{0:yyyyMMdd}.json", estDate ?? Provider.LocalNow);
		}

		#endregion

		#region Daily Projections: https://www.numberfire.com/nfl/fantasy/fantasy-football-projections/
		...
		protected override string DownloadProjectionsFromProvider(string fileKey)
		{
			try
			{
				...
				using (var client = new WebClient())
				{
					...
					foreach (var position in new[] { "QB", "RB", "WR", "TE", "K", "D" })
					{
						var content = client.DownloadString("https://www.numberfire.com/nfl/fantasy/fantasy-football-projections/" + position.ToLower());
		...
		{
			return string.Format("NFL_DailyProjections_{0:yyyyMMdd}.json", estDate ?? Provider.LocalNow);
		}
		...
		private DailyProjection GetBaseInfoDailyProjection(string nodeText)
		{
			...
			if (regTeamAndPosition.Success)
			{
				...
				{
					player.player = player.player.Replace("D/ST", "").Trim();
				}
			}
